...
class Task(console: Console[IO], client: Client[IO]) {
  ...
  def solve(): IO[Unit] = for {
    ...
    _ <- if (followerStars > stars) console.printLine(s"В репозитории $followerRepo $followerStars звёзд") else
...
object Task {
  def requestStr(repoName: String, method: String) = s"https://api.github/users/$repoName/$method"
